...
<p>The answer is: because this is what enables sync to work so well. PouchDB asks for a little pain up-front with managing document revisions, so that later on down the line, syncing is effortless.</p>
...
<p>PouchDB and CouchDB&#39;s document revision structure is very similar to Git&#39;s. In fact, each document&#39;s revision history is stored as a tree (exactly like Git), which allows you to handle conflicts in case any two databases get out of sync.</p>
...
<p>Deleting documents is actually very simple. The document is not really deleted; it just gets a <code>_deleted</code> attribute added to it.</p>
